...
public class SampleVid_Re : MonoBehaviour
{
    [SerializeField]
    ...
    void Start()
    {
        // m_originPos = Screen.transform.position;
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < m_height; i++)
        {
            for (int j = 0; j < m_width; j++)
            {
                ...
                Vector3 m_depthPos = new Vector3(m_pixels[i, j].transform.position.x, m_pixels[i, j].transform.position.y, m_depth);
