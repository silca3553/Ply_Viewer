...
public class Json_FirebaseInit : MonoBehaviour
{
    ...
    {
        var reference = storage.RootReference.Child($"{AvailableMaxIndex.ToString("000000")}.json");
        // mov 관련 스크립트 추가
        var reference2 = storage.RootReference.Child($"{AvailableMaxIndex.ToString("000000")}.mov");
        ...
        // mov 관련 스크립트 추가
        reference2.GetMetadataAsync().ContinueWith(task =>
        {
            AvailableMaxIndex++;
            if (task.IsCompletedSuccessfully)
                GetAvailableList();
    ...
    {
        var reference = storage.RootReference.Child($"{AvailableMaxIndex.ToString("000000")}.json");
        target_path = "file://" +basePath + "CameraInfo.json"; // 경로
    ...
    {
        int index =0;
        var reference = storage.RootReference.Child($"{index.ToString("000000")}.json");
        download_path = "file://" + basePath + "CameraInfo_receive.json"; // 경로
